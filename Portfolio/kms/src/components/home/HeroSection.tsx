...
export default function HeroSection({doc,career}:{doc:ContentDocument;career?:ContentDocument}){
 ...
 {career.meta.client&&<div className="identityCompany clientRow"><small>{labels.client}</small><strong>{String(career.meta.client)}</strong></div>}<div className="identityDivider"/>
 ...
 {doc.meta.image&&<div className="identityVisual"><Image src={String(doc.meta.image)} alt={String(doc.meta.imageAlt||"")} fill sizes="(max-width: 900px) 100vw, 320px" priority/></div>}</div>
